...
module.exports = async function (context, req) {
  ...
  try {
    ...
    const response = await fetch(
      `https://graph.microsoft.com/v1.0/users/${userKey}/appRoleAssignments?$select=id,appRoleId,resourceDisplayName`,
    ...
    const roles =
      data.value?.map((item) => ({
        ...
        resourceDisplayName: item.resourceDisplayName || 'Unknown app',
      })) ?? [];

    context.res = { status: 200, body: { count: roles.length, roles } };
  } catch (error) {
